...
func TestTimingWheel_Use(t *testing.T) {
	go printTicker()
	go use()
...
func printTicker() {
	...
	for {
		...
		case <-ticker.C:
			i++
			log.Println("已经过去了 ", i, " 秒")
		}
	}
}

func use() {
	tw := timingwheel.NewTimingWheel(time.Second*2, 10)
	tw.Start()
	tw.AfterFunc(time.Second*8, func() {
		log.Println(8)
	})
	...
	tw.AfterFunc(time.Second*4, func() {
		log.Println(4)
	})
